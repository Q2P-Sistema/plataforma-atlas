...
- **Valoração por rateio**: mantém-se a base de custo atual (valor da NF com tributos embutidos). Como o valor com tributos existe só no total da NF, ele é rateado entre os produtos proporcionalmente ao valor comercial de cada um (valor unitário × quantidade). Alternativa considerada e não adotada por padrão: usar o valor unitário comercial de cada item cru (sem tributos), o que reduziria a base de custo em relação ao praticado hoje. Decisão final da fórmula de rateio fica para a fase de plano.
