...
- [ ] T059 Executar o roteiro de `specs/015-recebimento-nacional-nf/quickstart.md`, cenários 1 a 8, contra o ambiente de desenvolvimento — **parcial em UAT (24/09/2026), com OMIE real**:
  ...
  - [ ] Cenário 2 (divergência de peso): nenhum recebimento com `quantidade_divergencia_kg <> 0` — é o coração da História 2 (motivo obrigatório + aprovação) e segue sem teste real.
  - [ ] Cenário 3 (1 item → N produtos): nenhum caso; o rateio ancorado na NF (D25) não foi exercitado.
  - [ ] Cenário 5 (unidade não conversível / incoerente bloqueia).
  - [ ] Cenário 6 (idempotência): re-POST da MESMA NF **sem** rejeitar (deve devolver `ja_recebido`) e o par de NFs de fornecedores diferentes com o mesmo número.
  - [ ] Cenário 8 (recebimento externo): nenhuma baixa solicitada; a aprovação de gestor e a reversão não foram exercitadas.
