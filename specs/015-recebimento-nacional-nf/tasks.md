...
- [x] T006c [P] Criar `modules/stockbridge/src/__tests__/auditoria-correlacao.test.ts` com um teste de integração que confirma gravação em `shared.audit_log` após INSERT e UPDATE em `stockbridge.correlacao_produto_fornecedor` — gate explícito do Princípio IV, hoje verificado só manualmente — *pula sem opt-in explícito (`ATLAS_DB_INTEGRATION=1` + `DATABASE_URL`) — o CI define `DATABASE_URL` dummy sem Postgres, então gatear só pela variável quebra o CI; roda contra banco real com a 0052 aplicada*
- [x] T007 Aplicar e validar localmente com `pnpm --filter @atlas/db migrate`, confirmando que os dois índices únicos foram criados, que a trigger grava em `shared.audit_log` e que as duas linhas de exclusão de fornecedor ficaram ativas em `stockbridge.fornecedor_exclusao` — **aplicada em UAT em 18/09/2026** (não em DEV, que não tinha credencial na sessão) e verificada em leitura: extensão `unaccent`, tabela, os dois índices únicos, CHECKs de `recebimento_externo` e as duas exclusões ativas. A **trigger ficou provada em uso** (24/09): 6 linhas em `shared.audit_log` para `correlacao_produto_fornecedor` geradas pelos recebimentos reais
...
- [x] T009 [P] Criar `modules/stockbridge/src/__tests__/unidade-nf.test.ts` cobrindo as 3 unidades conversíveis, o bloqueio por unidade desconhecida (`UN`), o **bloqueio por incoerência** (NF 58067: `q_com=1,375`, `u_com='KG'` → R$ 14.350/kg lido como KG, R$ 14,35/kg lido como tonelada — números de `v_prod`, D26) e a **não-regressão de material barato** (papelão a R$ 0,35/kg em KG e sucata a R$ 300/t em TON seguem liberados), caixa/espaços na grafia, e a ausência de conversão implícita em qualquer caminho
...
- [x] T037 [US3] Criar `modules/stockbridge/src/services/correlacao-produto.service.ts` com a normalização de descrição (trim, colapso de espaços, caixa alta, remoção de acentuação — data-model.md §1.1) aplicada tanto na gravação quanto na consulta
- [x] T038 [US3] Implementar em `modules/stockbridge/src/services/correlacao-produto.service.ts` a consulta de sugestão: dado (fornecedor, descrição normalizada), devolver os produtos ativos ordenados por `vezes_usada DESC`
- [x] T039 [US3] Implementar em `modules/stockbridge/src/services/correlacao-produto.service.ts` a gravação da correlação, incrementando `vezes_usada` e `ultima_vez_usada_em` quando a sugestão é aceita, e registrando `atualizado_por` na correção
- [x] T040 [US3] Ligar a sugestão ao detalhe da NF em `fila-nacional.service.ts`, preenchendo `produtosSugeridos` por item e marcando `bloqueio: 'sem_correlacao'` quando o par é inédito (FR-005, FR-006)
- [x] T041 [US3] Gravar a correlação automaticamente ao concluir o recebimento, em `modules/stockbridge/src/services/recebimento-nacional.service.ts`, para que a escolha do operador valha nas próximas NFs (FR-007)
- [x] T042 [US3] Adicionar a rota `PUT /recebimento/nacional/correlacao` em `modules/stockbridge/src/routes/recebimento-nacional.routes.ts`, conforme contrato §4, com `requireOperador`
- [x] T043 [US3] Pré-selecionar o produto sugerido em `apps/web/src/pages/stockbridge/operador/RecebimentoNacionalNfPanel.tsx` e permitir a troca, mantendo o combobox existente de `/recebimento/nacional/produtos` como caminho de busca
- [x] T044 [P] [US3] Criar `modules/stockbridge/src/__tests__/correlacao-produto.test.ts` cobrindo a normalização (incluindo os casos que fundem grafias — research D8), a sugestão de par conhecido, o par inédito sem sugestão, e a correção por UPDATE auditado
...
- [x] T049 [US4] Estender `correlacao-produto.service.ts` para memorizar o **conjunto** de produtos por descrição, desativando (`ativo = false`, auditado) os que saírem do conjunto — nunca `DELETE`
- [x] T050 [US4] Permitir em `apps/web/src/pages/stockbridge/operador/RecebimentoNacionalNfPanel.tsx` adicionar e remover produtos de um item, com a distribuição de quantidade e um indicador de quanto falta para fechar
...
- [x] T064 [US6] Adicionar a flag `STOCKBRIDGE_RECEBIMENTO_EXTERNO_ENABLED` (default `true`) em `packages/core/src/config.ts`, no padrão de `STOCKBRIDGE_BAIXA_PEDIDO_Q2P_ENABLED`
- [x] T065 [US6] Criar `modules/stockbridge/src/services/recebimento-externo.service.ts` que grava em `stockbridge.aprovacao` com `tipo_aprovacao='recebimento_externo'`, `precisa_nivel='gestor'`, `movimentacao_id` e `lote_id` nulos, e as colunas de NF (`nf_chave_acesso`, `nota_fiscal`, `nf_item_descricao`). Motivo obrigatório em `observacoes`
- [x] T066 [US6] Garantir em `recebimento-externo.service.ts` que a aprovação **não** cria movimentação, **não** altera estoque e **não** chama o OMIE — é o invariante 9 do contrato e a única salvaguarda contra a ação virar um sumidouro de trabalho
- [x] T067 [US6] Adicionar em `modules/stockbridge/src/services/aprovacao.service.ts` o ramo de `tipo_aprovacao='recebimento_externo'` na função `aprovar()`, que apenas muda o status — sem passar pelo caminho de ajuste OMIE de `aprovarEntradaNacional`
- [x] T068 [US6] Adicionar a rota `POST /recebimento/nacional/recebimento-externo` em `modules/stockbridge/src/routes/recebimento-nacional.routes.ts` conforme contrato §5, recusando com `403 RECEBIMENTO_EXTERNO_DESABILITADO` quando a flag estiver desligada
- [x] T069 [US6] Refletir a baixa na fila em `modules/stockbridge/src/services/fila-nacional.service.ts`: item com `recebimento_externo` **aprovado** sai da fila; com solicitação **pendente**, continua visível marcado como "baixa solicitada"; **rejeitada**, volta a pendente normal
- [x] T069a [US6] Adicionar `'recebimento_externo'` ao mapa `TIPO_LABEL` de `apps/web/src/pages/stockbridge/gestor/AprovacoesPage.tsx` e renderizar o card com NF, descrição do item e motivo — sem produto e sem painel de divergência. Hoje o fallback imprime o identificador cru e o título cai em `SKU 0`, violando FR-015
- [x] T070 [US6] Adicionar a ação na UI (`apps/web/src/pages/stockbridge/operador/RecebimentoNacionalNfPanel.tsx`) com motivo obrigatório e atalho de "todos os itens pendentes", escondida quando a flag estiver desligada
- [x] T071 [P] [US6] Criar `modules/stockbridge/src/__tests__/recebimento-externo.test.ts` cobrindo: motivo ausente recusa; aprovação retira da fila; **nenhuma movimentação criada**; rejeição devolve à fila; item já recebido recusa com `ITEM_JA_RECEBIDO`; flag desligada recusa a rota
- [x] T071a [US6] Implementar a reversão em `modules/stockbridge/src/services/recebimento-externo.service.ts`: função que devolve uma aprovação `recebimento_externo` de `aprovada` para `pendente` com motivo, auditada. **Decisão na implementação**: a reversão grava `status='rejeitada'` com `rejeicao_motivo='Reversão: <motivo>'` (não volta a `pendente`, o que recolocaria uma solicitação na caixa do gestor); o efeito na fila é o mesmo — só `aprovada` retira o item. Hoje nada no código reverte uma aprovação já aprovada — `aprovacao.service.ts` não tem função que devolva status a `pendente`, e `listarPendencias` filtra `status = 'pendente'`, então a baixa aprovada nem aparece ao gestor (FR-031)
- [x] T071b [US6] Expor a reversão em `modules/stockbridge/src/routes/aprovacao.routes.ts` (`POST /aprovacoes/:id/reverter`, gestor+) e listar as baixas externas aprovadas em `apps/web/src/pages/stockbridge/gestor/AprovacoesPage.tsx`, sem o que não há como acionar a reversão
- [x] T072 [P] [US6] Cobrir em `modules/stockbridge/src/__tests__/recebimento-externo.test.ts` a reversão pelo gestor de uma baixa já aprovada, devolvendo o item à fila, com registro em `shared.audit_log`
...
- [x] T057 [P] Rodar `pnpm lint && pnpm typecheck` e corrigir violações, incluindo `eslint-plugin-boundaries` (gate bloqueante — Princípio I)
- [x] T058 [P] Rodar a suíte completa `pnpm --filter @atlas/stockbridge test` e confirmar que nenhum teste existente do fluxo de importação quebrou — *496 passam, 5 pulam (integração sem banco); `metricas`/`debito-cruzado` falham por ambiente (carregam `@atlas/core` real sem `DATABASE_URL`) — provado idêntico sem as mudanças via `git stash`*
- [ ] T059 Executar o roteiro de `specs/015-recebimento-nacional-nf/quickstart.md`, cenários 1 a 8, contra o ambiente de desenvolvimento — **parcial em UAT (24/09/2026), com OMIE real**:
  - [x] Cenário 1 (receber sem digitar): 6 recebimentos em 5 NFs; 4 aprovados e **concluídos no OMIE**. Valor confere com a NF-e — 59697 gravou R$ 267.300,15 e R$ 10,80/kg, o `<vNF>` exato do XML (prova do D26 em uso).
  - [x] Cenário 4 (correlação memorizada): 4 pares gravados, `SUCATA PP COLORIDO` já com `vezes_usada = 3` — a sugestão foi reusada, não só criada. Trigger de auditoria com 6 registros.
  - [x] Ciclo de **rejeição e re-recebimento** (não estava no roteiro): gestor rejeitou a NF 36537, a movimentação foi para `ativo = false` liberando o índice de idempotência, e a NF foi recebida de novo e aprovada 53 min depois.
  - [x] Cenário 2 (divergência de peso) — **OK em 25/09**, nos dois sentidos.
  - [x] Cenário 3 (1 item → N produtos) — **OK em 25/09**; o rateio ancorado na NF (D25) exercitado.
  - [x] Cenário 7 (o caminho manual não regrediu): 2 recebimentos pelo formulário manual em 24/09, já com a feature no ar.
  - [x] Cenário 8 (recebimento externo) — **OK em 25/09**, incluindo aprovação e reversão.
  - [x] Cenário 5, unidade **não conversível** — **OK em 25/09** (NF 67305, Ecologika, `u_com = UN`).
  - [ ] Cenário 5, unidade **incoerente** (KG com quantidade em tonelada): **sem caso testável em UAT**. Varrendo o espelho desde maio/2026, todos os itens nessa condição já foram recebidos pelo formulário manual e a checagem de duas vias os retira da fila — foi o que aconteceu com as NFs 59311/59321 da Zaraplast quando o corte foi recuado para 28/08, validando de quebra o **SC-008**. Coberto por Vitest (`unidade-nf.test.ts`, números reais da NF 58067); fica pendente de observação quando surgir uma NF nova nessa condição.
  - [x] Cenário 6 (idempotência) — **fechado em 25/09**. Por NF: validado na tela, a NF recebida não reaparece na fila. Por item: **não é exercitável pela tela por decisão de produto** (research D27, 25/09) — a NF é recebida por inteiro, sem seleção parcial, e o recebimento parcial só acontece por falha de gravação. Fica coberto por Vitest (`recebimento-nacional-nf.test.ts`, retomada em duas levas) e pelo índice único da migration 0052.
  - [ ] Colisão de número entre fornecedores (FR-013): **não testável em UAT** — os únicos pares no espelho (`5682`, `6936`, `6937`, `5624`) têm PLASTFIX ou a ACXE intercompany como contraparte, ambas excluídas da fila por escopo. Coberto por Vitest e pelo índice único, que usa a chave de acesso.
- [x] T060 [P] Revisar as mensagens de usuário de `apps/web/src/pages/stockbridge/operador/RecebimentoNacionalNfPanel.tsx` e `modules/stockbridge/src/routes/recebimento-nacional.routes.ts` com o agente `frontend-design-reviewer`, conferindo tom institucional em pt-BR e ausência de código OMIE. **Feito 18/09/2026**: 2 bloqueantes corrigidos (código ACXE exposto como "SKU" no card de aprovação sem lote e código do produto no dropdown do `ProdutoCombobox`), mais: nome de variável de ambiente fora da `userMessage` da data de corte, mensagem própria para NF localizada sem item no recorte de CFOP, "espelho" trocado por "sincronizada", `userMessage` institucional em todo `INVALID_INPUT`, `ItemNaoCorrespondeError` (400) em vez de `Error` genérico (500), fallback "SKU <código>" do digest manual removido, legenda/rotulo uniformizados
- [x] T061 [P] Atualizar a seção StockBridge de `CLAUDE.md` com um parágrafo sobre o recebimento nacional por NF: identidade por chave de acesso, divergência aceita nos dois sentidos, classificação 1:N e a janela temporal da fila
- [x] T062 Conferir a fila contra PROD usando a query final de `quickstart.md` (corte de 7 dias) — espera-se ordem de **unidades** de NFs, não dezenas; na medição de 17/09/2026 foram 9 elegíveis, das quais 2 realmente pendentes. **Medição 18/09/2026 (pg-acxe, leitura)**: 8 NFs elegíveis (8 chaves distintas, emissão 11–17/09), todas com 1 item; 7 em `KG` e 1 em `TL` (Zaraplast 59626) — confirma a ordem de grandeza e a cobertura da tabela de unidades. A parcela "realmente pendente" só é mensurável em UAT (o espelho PROD não tem `stockbridge.*`)
- [x] T063 Registrar no Jira (ACXEGDP-328), com base em `specs/015-recebimento-nacional-nf/research.md` D13, o passivo das NFs 66529/66530/66604 como item separado, com os ~40,7 t de excesso já aplicados no OMIE — a feature impede repetição, não corrige o histórico — *feito em 18/09: comentário em ACXEGDP-328; o passivo virou correção aplicada (script versionado), sem card separado*
- [x] T073 **Correção pós-UAT (24/09/2026, research D26)**: trocar `i.v_tot_item` por `i.v_prod` em `fila-nacional.service.ts` (fila, detalhe, valor do item, R$/kg e, por consequência, `custo_unitario_brl` gravado). Achado no 1º recebimento real em UAT — NF 59697 da Zaraplast mostrava R$ 280.028,73 contra `<vNF>` 267.300,15 no XML (IPI de 12.728,58 em dobro). Provado em PROD: 1.431 de 1.663 linhas com o tributo somado duas vezes, nenhuma em que `v_tot_item` seja o certo; a conferência de unidade (D24) não muda de veredito em nenhum item. Teste de regressão com os números reais da NF em `fila-nacional.test.ts`. As 2 movimentações já gravadas em UAT (36537/36565, sucata sem IPI) estão corretas e não precisam de ajuste. As descrições de T015/T017/T027/T047/T051 acima citam `v_tot_item` por serem históricas
