...
- [ ] T022 [P] Verificar performance do endpoint: `time curl -X POST http://localhost:3000/api/v1/stockbridge/admin/nf-pedido-mapa` com payload de 50 pedidos (cada um com 2–3 filhotes) — confirmar resposta < 5 segundos (SC-003)
- [ ] T023 Commit e push da branch `010-fiscal-nf-mapa` com todas as mudanças
- [ ] T024a [P] Confirmar que o briefing n8n no `plan.md` está atualizado (texto pronto para passar ao agente) — extensão do workflow `hP7OrMQEs2av8Lj7` para ler aba "NF ENTRADA" e chamar `POST /admin/nf-pedido-mapa`
- [ ] T024b **[DEPENDÊNCIA EXTERNA]** Executar extensão do workflow n8n `hP7OrMQEs2av8Lj7` — a ser realizado pelo agente n8n após endpoint estar no ar em UAT; confirmar via `GET /admin/nf-pedido-mapa` que os pedidos da aba "NF ENTRADA" foram importados
