...
- **FR-003**: O cálculo da "Posição Fiscal Pendente de Importação" no cockpit DEVE usar o mapa quando disponível: pedido pendente se qualquer filhote não tiver sido recebida pelo ERP (`n_id_receb = 0` ou ainda não sincronizada). Mapa com filhotes ainda não cadastradas no sistema também é tratado como pendente.
