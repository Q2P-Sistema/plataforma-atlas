...
router.get(
  "/api/v1/stockbridge/baixa-pedido/:id",
  requireGestor,
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error(
        { err, movimentacaoId: params.data.id },
        "Erro ao listar ledger da baixa",
      );
      res
        .status(500)
        .json({
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error(
        { err, movimentacaoId: params.data.id },
        "Erro ao retentar baixa de pedido Q2P",
      );
      res
        .status(500)
        .json({
