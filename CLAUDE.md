...
- **Baixa do pedido de compra Q2P após recebimento (ACXEGDP-344, migration 0047)**: o legado PHP reduzia o saldo do pedido de compra da Q2P no OMIE (`AlteraPedCompra`, FIFO por produto) após cada recebimento de importação; o Atlas tinha o cliente (`alterarPedidoCompra`, T016) mas **nunca o chamava** — 3,5 kt recebidas entre 09/06 e 24/08/2026 ficaram sem baixa. Agora `baixa-pedido.service.ts` roda **após o dual concluir** (recebimento limpo, aprovação de divergência e retries de `pendente_q2p`/`pendente_acxe_faltando`), best-effort: `movimentacao.baixa_pedido_q2p` = `pendente → concluida | sem_saldo | falha`; ledger `stockbridge.baixa_pedido_q2p` por (movimentação, pedido) com saldo anterior/novo. Regras: candidatos enumerados no espelho `tbl_pedidosCompras_Q2P` (etapa 15 = aberto, 1 item por pedido, `cobs` traz "Pedido original ACXE: N"), **saldo lido ao vivo** via `ConsultarPedCompra` antes de cada alteração (o espelho sincroniza 1×/dia e o AlteraPedCompra manda quantidade ABSOLUTA), **sem FIFO** (decisão 28/08/2026, migration 0051): a baixa só acontece no pedido Q2P casado com o pedido ACXE da NF filhote via `nf_pedido_mapa` **inclusive linhas inativas** (o mapa é auto-desativado quando a NF é recebida — inativo = recebida, não vínculo inválido; filtrar por ativo perdia 137/150 vínculos); sem vínculo → `aguardando_vinculo` e o cron horário (`10 * * * *`, após a carga da FUP pelo n8n) re-tenta, digest à Comex após 3 dias; excedente sobre o pedido vinculado → `sem_saldo` + alerta, nunca transborda para outro pedido; `criterio` (`vinculo_nf|fifo`) gravado no ledger (migration 0048); reversão auditável `desfazerBaixaPedidoQ2p` / `--desfazer <nf> --motivo`, sentinela **0,1 kg** para pedido zerado (OMIE rejeita 0 — paridade com o legado), lock consultivo por produto, ledger `pendente` gravado ANTES da chamada (retry confere o saldo ao vivo com o alvo e não desconta duas vezes). Painel/retry gestor+ em `GET /api/v1/stockbridge/baixa-pedido/pendentes` e `POST /baixa-pedido/:movimentacaoId/retentar` (coluna "Pedido Q2P" em Movimentações). Flag `STOCKBRIDGE_BAIXA_PEDIDO_Q2P_ENABLED` (default true). Backfill: `pnpm --filter @atlas/stockbridge backfill-baixa-pedido-q2p` (dry-run com saldos encadeados; `--execute` aplica; `--consultar <nCodPed>` valida o parse em UAT).
