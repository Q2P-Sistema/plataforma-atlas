...
interface ItemNf {
  ...
  produtosSugeridos: Array<{ codigo: number; descricao: string; vezesUsada: number }>;
  bloqueio: Bloqueio;
    ...
    <div>
      <div className="flex items-center justify-between mb-2 gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            NF fora da lista? Registrar manualmente
          </button>
        </div>
      </div>
            ...
            return (
              <div key={x.nfChaveAcesso} className="bg-atlas-card border border-atlas-border rounded-lg p-4 flex items-center gap-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-[11px] text-atlas-muted">valor da nota</div>
                </div>
                <button
...
function DetalheSection({ chave, onVoltar, onAbrirManual }: { chave: string; onVoltar: () => void; onAbrirManual: () => void }) {
  ...
  const getForm = (i: number): ItemForm =>
    form[i] ?? { produto: null, localidadeId: '', quantidadeConferida: '', motivoDivergencia: '' };
  const setItem = (i: number, patch: Partial<ItemForm>) => setForm((prev) => ({ ...prev, [i]: { ...getForm(i), ...patch } }));

  /** Itens que o operador pode receber por aqui (nao bloqueados, nao recebidos por inteiro). */
  const recebiveis = useMemo(
    () =>
      (nf?.itens ?? []).filter(
        (it) => it.quantidadeNfKg != null && it.bloqueio !== 'unidade_nao_conversivel' && it.bloqueio !== 'unidade_incoerente' && !(it.jaRecebido && (it.quantidadeRestanteKg ?? 0) <= TOLERANCIA_KG),
      ),
    [nf],
  );
  ...
  function validar(): string | null {
    ...
    for (const it of recebiveis) {
      const fi = getForm(it.indice);
      const rot = `"${it.descricaoFornecedor.trim()}"`;
      if (!fi.produto) return `Escolha o produto para ${rot}.`;
      if (!fi.localidadeId) return `Escolha o estoque destino para ${rot}.`;
    ...
    mutationFn: async () => {
      const payload = {
        ...
        itens: recebiveis.map((it) => {
          const fi = getForm(it.indice);
          const conferida = parseNum(fi.quantidadeConferida);
          const kg = conferida ?? it.quantidadeNfKg ?? 0;
          const restanteConferida = Math.max(0, kg - it.quantidadeConferidaJaGravadaKg);
          return {
            indice: it.indice,
            ...
            produtos: [{ produto_codigo_q2p: fi.produto!.codigo, quantidade_kg: restanteConferida, localidade_id: fi.localidadeId }],
          };
        }),
      };
          ...
          <div className="flex flex-col gap-3">
            {nf.itens.map((it) => (
              <ItemCard
                key={it.indice}
                item={it}
                form={getForm(it.indice)}
                localidades={localidades}
                onChange={(patch) => setItem(it.indice, patch)}
              />
            ))}
          </div>
...
function ItemCard({ item, form, localidades, onChange }: { item: ItemNf; form: ItemForm; localidades: Localidade[]; onChange: (p: Partial<ItemForm>) => void }) {
  ...
  return (
    <div className={`bg-atlas-card border rounded-lg p-4 ${bloqueadoUnidade ? 'border-amber-300 dark:border-amber-800' : 'border-atlas-border'}`}>
      ...
      {!bloqueadoUnidade && !recebidoIntegral && (
        <div className="mt-3 grid grid-cols-1 md:grid-cols-12 gap-3">
          <div className="md:col-span-5">
            <label className="block text-xs font-medium text-atlas-muted mb-1">Produto no estoque</label>
            <ProdutoCombobox empresa="q2p" valor={form.produto} onChange={(p) => onChange({ produto: p })} />
          </div>
          <div className="md:col-span-3">
            <label className="block text-xs font-medium text-atlas-muted mb-1">Estoque destino</label>
            <select
              value={form.localidadeId}
              onChange={(e) => onChange({ localidadeId: e.target.value })}
